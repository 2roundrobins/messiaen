...
-- bird tables
bird_tab = {bird.wren, bird.robin, bird.trush, bird.gale, bird.blackbird, bird.finch, bird.g_tit}
...
function redraw()
  ...
  if garden_is_planted then
    ...
    if bird_is_singing and not (info or garden_is_planted) then
      screen.display_png(_path.code .. "/massiaen/assets/brd_pngs/note.png", 90, 20)
    end
    
    if is_recording and not (info or garden_is_planted) then
    ...
    elseif active_bird_name =="finch" then
    ...
    elseif active_bird_name == "wren" then --- this is new use it at every brd change!
      if info == true then
        display_exl = false
        screen.clear()
